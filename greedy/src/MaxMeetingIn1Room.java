...
public class MaxMeetingIn1Room {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (testCases-- != 0) {
            ...
            for (int i = 0; i < n; i++) {
                meetings.add(new Meet(Integer.parseInt(s1[i]), Integer.parseInt(s2[i]), i + 1));
            }
            Collections.sort(meetings, Comparator.comparingInt(o -> o.end));
            int timeLimit = meetings.get(0).end;
            s.append(meetings.get(0).pos + " ");
            for (int i = 1; i < n; i++) {
                if (timeLimit <= meetings.get(i).start) {
                    s.append(meetings.get(i).pos + " ");
